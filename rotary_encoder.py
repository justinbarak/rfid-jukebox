# https://github.com/MiczFlor/RPi-Jukebox-RFID/blob/develop/components/gpio_control/GPIODevices/rotary_encoder.py
#!/usr/bin/env python3
# rotary volume knob
...
class RotaryEncoder:
    ...
    def __init__(
        ...
        pinA,
        pinB,
        functionCallIncr=None,
        functionCallDecr=None,
        timeBase=0.1,
        name="RotaryEncoder",
    ):
        logger.debug(
            "Initialize {name} RotaryEncoder({arg_Apin}, {arg_Bpin})".format(
                arg_Apin=pinA, arg_Bpin=pinB, name=name if name is not None else ""
            )
        )
        self.name = name
        # persist values
    ...
    def __repr__(self):
        repr_str = (
            "<{class_name}{object_name} on pin_a {pin_a},"
    ...
    def _Callback(self, pin):
        ...
        if self.KeyIncr == self.encoderState.asByte:
            ...
            logger.info(
                "{name}: Calling functionIncr {steps}".format(
            ...
            steps = self._StepSize()
            logger.info(
                "{name}: Calling functionDecr {steps}".format(
                    name=self.name, steps=steps
                )
            )
