# https://github.com/MiczFlor/RPi-Jukebox-RFID/blob/develop/components/gpio_control/GPIODevices/simple_button.py
import time
...
map_pull_print = {
    GPIO.PUD_UP: "pull_up",
    GPIO.PUD_DOWN: "pull_down",
    GPIO.PUD_OFF: "pull_off",
}
...
def parse_pull_up_down(pull_up_down):
    ...
    try:
        ...
        result = pull_up_down
        raise KeyError(
            "Unknown Pull Up/Down type {pull_up_down}".format(pull_up_down=pull_up_down)
        )
    return result
...
class SimpleButton:
    def __init__(
        ...
        GPIO.add_event_detect(
            self.pin,
            edge=self.edge,
            callback=self.callbackFunctionHandler,
    ...
    def callbackFunctionHandler(self, *args):
        if (
            len(args) > 0
            and args[0] == self.pin
            and not self.callback_with_pin_argument
        ):
            logger.debug(
                "Remove pin argument by callbackFunctionHandler - args before: {}".format(
                    args
                )
            )
            args = args[1:]
